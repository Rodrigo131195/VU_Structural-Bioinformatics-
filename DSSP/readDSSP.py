...
def read_dir(d, unfolded_acc):
    ...
    """
    # Initialize dictionaries to count all amino acids and buried amino acids
    all_aa_count = {}
    buried_aa_count = {}
...
def decide_if_buried(aa, acc, unfolded_acc):
    ...
    unfolded_accessibility = unfolded_acc[aa]

    # Calculate fraction of buried SA
    burial_fraction = acc / unfolded_accessibility

    # Check if fraction < 7%
    if burial_fraction < 0.07:
        buried = True
...
def print_propensities(all_aa_count, buried_aa_count, outfile):
    ...
    # print(all_aa_count, buried_aa_count)

    ### START CODING HERE
    # Calculate counts of all residues
    total_residues_count = sum(all_aa_count.values())
    total_buried_residues_count = sum(buried_aa_count.values())

    # Iterate over each amino acid
    for aa in list_aa:
        # Calculate the propensity of burial
        propensity_buried = (buried_aa_count[aa] / all_aa_count[aa]) / (
                    total_buried_residues_count / total_residues_count)
